...
`pnpm start`는 "지금"만 본다. 과거는 소스마다 받는 방법이 달라 별도 명령으로 뺐다.
둘 다 결과가 거의 바뀌지 않으므로 매 실행마다 돌 이유가 없다.

**`backfill:permits`** — 서울시 부동산정보광장은 **한 번에 62일까지만** 돌려주고, 그 이상을
요청하면 에러 없이 빈 결과를 준다(실측: 62일 410건, 63일 0건). 그대로 두면 "허가가 없었다"로
둔갑하므로 수집기가 넘치는 요청을 아예 보내지 않는다. 62일은 조회 폭의 한계지 보관 기간이
아니라서, 창으로 쪼개면 2023년 자료까지 읽힌다.

**`backfill:kb`** — KB의 `mpriByType`은 "지금" 값만 준다. 그래서 KB 곡선이 수집을 시작한 날부터만
생기고, 관심 면적을 나중에 추가하면 그 면적만 짧게 잘린다. KB는 같은 값을 월별로도 들고 있어
(`QuotBaseYear` → `WholQuotList`) 한 번 받아 두면 관측 이전 구간이 메워진다. **면적을 새로
추가했을 때 한 번 돌려 주면 된다.**

### 계열별로 과거를 어디까지 캘 수 있나

차트의 계열마다 이력의 한계가 다르다. "왜 이 선만 짧지?"를 다시 파지 않도록 적어 둔다.

| 계열 | 출처 | 과거 | 한계 |
|---|---|---|---|
| 실거래 | 국토교통부 | 소급 조회 가능 | 거래가 있던 달부터. 신축은 전매제한이 풀린 뒤부터 생긴다 |
| KB 일반가·하위~상위 | KB부동산 | `backfill:kb`로 월별 | KB가 시세를 매기기 시작한 달까지. 신축은 그 전이 아예 없다 |
| 토지거래허가 | 서울시(국토부 K-Geo 연계) | `backfill:permits`로 62일씩 | 서울시만. 자치구 단위 조회 |
| **호가 최저~최고** | 네이버 부동산 | **불가** | 우리가 `ask_snapshots`에 쌓는 것이 유일하다 |

호가만 유독 방법이 없다. 네 갈래를 확인했다 — 네이버 시세 차트 API는 응답이 `chartProvider:
"kbstar"`로 KB 시세를 받아 그리는 것이고, 매물 상세의 `prevPrice`는 이력이 아니라 직전값 하나이며,
내려간 매물은 조회 자체가 안 되고, 통계 계열 엔드포인트는 전부 404다. 그래서 관심 면적을 새로
추가하면 그 면적의 호가 밴드는 짧을 수밖에 없고, 시간이 쌓이는 것 말고는 방법이 없다.
