...
export async function runTelegramReport(): Promise<void> {
  ...
  for (const { apt, target, transactions, diff, kbPrice } of collected) {
    try {
      await sendNotification(apt.name, target.area, transactions, diff, kbPrice);
    } catch (err) {
      console.error(`[${apt.name} ${target.area}㎡] 알림 발송 실패:`, err);
    }
    await delay(1000);
  }
}
